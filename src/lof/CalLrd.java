...
public class CalLrd {
	...
	public static class CalLRDMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
		 ...
		 * key: nid || value: partition id, k-distance, whoseSupport,(KNN's nid and dist) 
		 * 
		 * output format: 
		 * (Core area)key: partition id || value: nid, type(S or C), k-distance,whoseSupport, (KNN's nid and dist) 
		 * (Support area)key: partition id || value: nid, type(S or C), k-distance
		 * 
		 * @author yizhouyan
		 */
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			interValue.set(nid + ",C," + kdist + "," + strValue[2] + "," + knn_id_dist);
	...
	public static class CalLRDReducer extends Reducer<IntWritable, Text, LongWritable, Text> {
		...
		/**
		 * used to calculate LRD input format: 
		 * (Core area)key: partition id || value: nid, type(S or C), k-distance, whoseSupport, (KNN's nid and dist) 
		 * (Support area)key: partition id || value: nid, type(S or C), k-distance
		 * 
		 * @author yizhouyan
		...
		private MetricObject parseObject(int key, String strInput) {
			...
			int offset;
			if (type != 'S') {
				offset = inputSplits[0].length() + inputSplits[2].length()
						+ inputSplits[3].length() + 5;
				KNN = strInput.substring(offset, strInput.length());
				whoseSupport = inputSplits[3];
			}
			return new MetricObject(partition_id, obj, kdistance, KNN, type, whoseSupport);
		}
		...
		private void CalLRDForSingleObject(Context context, MetricObject o_S, HashMap<Long, Float> hm)
			...
			for (int i = 0; i < splitKNN.length; i++) {
				...
				float temp_reach_dist = Math.max(temp_dist, hm.get(Long.valueOf(tempSplit[0])));
				lrd_core += temp_reach_dist;
			}
			lrd_core = 1.0f / (lrd_core / K * 1.0f);
			String line = "";
			// output format key:nid value: partition id, lrd, whoseSupport, (KNN's nid and dist)
			line += o_S.getPartition_id() + SQConfig.sepStrForRecord + lrd_core
					+ SQConfig.sepStrForRecord + o_S.getWhoseSupport() + SQConfig.sepStrForRecord + o_S.getKnnsInString();
	...
	public void run(String[] args) throws Exception {
		...
		/** set job parameter */
		Job job = Job.getInstance(conf, "DDLOF: Calculate lrd");
